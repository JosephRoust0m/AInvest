class AuthAPI {
  ...
  async signIn(credentials) {
    ...
    async signInAdvisor(credentials) {
    try {
      const response = await fetch(this.baseURL+`/api/advisor/sign-in`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  async sendLogoutTimestampAdvisor(username) {
    try {
      const response = await fetch(this.baseURL+`/api/advisor/logout-timestamp`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('authToken')}`
