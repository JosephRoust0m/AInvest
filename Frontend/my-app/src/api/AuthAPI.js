class AuthAPI {
  ...
  async getUserLastLogout(username) {
    try {
      const response = await fetch(`${this.baseURL}/api/user/last-logout?username=${username}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('authToken')}`
  ...
  async expertSignIn(credentials) {
    try {
      const response = await fetch(`${this.baseURL}/api/expert/sign-in`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || 'Expert sign in failed');
      }

      const result = await response.json();
      console.log('Expert sign in response:', result);
      // Store the authentication token if provided
      if (result.token || result.accessToken || result.authToken) {
        const token = result.token || result.accessToken || result.authToken;
        localStorage.setItem('authToken', token);
  ...
  async sendExpertLogoutTimestamp(expertUsername) {
    try {
    
      console.log('Sending expert logout timestamp for username:', expertUsername);
      const response = await fetch(`${this.baseURL}/api/expert/logout-timestamp`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('authToken')}`
  ...
  async getExpertLastLogout(expertUsername) {
    try {
      const response = await fetch(`${this.baseURL}/api/expert/last-logout?username=${expertUsername}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('authToken')}`
